...
@pytest.mark.asyncio
async def test_chat_model_openai():
    ...
    temperature = 0
    max_tokens = 4000
    openai_model = OPENAI_Model(model=model, temperature=temperature, max_tokens=max_tokens)

    actual_result = await openai_model.get_chat_response(AGENT_ROLE_PROMPT, prompt)
